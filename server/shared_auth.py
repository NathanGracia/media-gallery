...
connecté sur Blindtoss/Memoss sans repasser par un login local.

Port Python du schéma défini dans cooloss/lib/sharedToken.ts — si le format
change là-bas, réplique le changement ici. Vérification stateless (HMAC
local, pas d'appel réseau à cooloss) : si cooloss tombe, les sessions déjà
ouvertes continuent de fonctionner.
...
class SharedClaims(TypedDict):
    ...
    # Volume général (0..1), partagé avec Shardoss/Blindtoss/cooloss — voir
